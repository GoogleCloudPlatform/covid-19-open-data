    ...
    def post_error_to_github(self, error_group):
        ...
        if response.status_code != 201:
            raise ConnectionError(
                "Could not create github issue, bad status code received: {}".format(
...
def register_new_errors():
    ...
    for error_group in errors_past_day["errorGroupStats"]:
        ...
        if not doc.get().exists and error_group["group"]["resolutionStatus"] == "OPEN":
            try:
                error_group["group"]["trackingIssues"] = gh_issue_handler.post_error_to_github(
                    error_group
                )
